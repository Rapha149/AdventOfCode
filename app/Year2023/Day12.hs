module Year2023.Day12 (part1, part2) where
...
part1 :: Solution
part1 = V . sum . map (uncurry possibleCombinations . (\(a, b) -> (a, map read $ splitOn "," b)) . tuple . words)

part2 :: Solution
part2 = V . sum . map (uncurry possibleCombinations . (\(a, b) -> (intercalate "?" $ replicate 5 a, concat $ replicate 5 $ map read $ splitOn "," b)) . tuple . words)
