...
data OCRResult = Parsed String | OCRError String
...
parseOCR :: [[Bool]] -> (String, OCRResult)
parseOCR rows | null letterMap = (ocr, OCRError "Invalid letter height.")
              | otherwise = (ocr, parse $ transpose rows)
    where ocr = trimEnd $ unlines $ map (map (\case True -> '#'; False -> ' ')) rows
          ...
          parse cols = case Map.toList $ Map.filter (`isPrefixOf` cols) letterMap of
                            [(letter, prefix)] -> case parse $ drop (length prefix) cols of
                                                       ...
                                                       err -> err
                            [] -> OCRError "No matches."
                            xs -> OCRError ("Multiple matches: " <> intercalate ", " (map (singleton . fst) xs))
