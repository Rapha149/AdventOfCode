module Years.Year2024.Day11 (part1, part2) where
...
changeNumber k v | even $ length str = Map.insertWith (+) a v . Map.insertWith(+) b v
                 | otherwise = Map.insertWith (+) (k * 2024) v
    where str = show k
          (a, b) = both read $ splitAt (length str `div` 2) str

countAfter :: Int -> [String] -> Int
countAfter blinks input = sum $ foldr ($) numbers (replicate blinks (Map.foldrWithKey changeNumber Map.empty))
