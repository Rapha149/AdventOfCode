module Years.Year2024.Day24 (part1, part2) where
...
parseInput input = (Map.fromList [(wire, read val) | [wire, val] <- map (splitOn ": ") wireInput],
                    Map.fromList $ map (parseGate . words) gateInput)
    where [wireInput, gateInput] = split null input
...
part1 :: Solution
part1 input = V $ foldr (\i acc -> i + acc * 2) 0 zValues
    where (wires, gates) = parseInput input
...
part2 :: Solution
part2 input = Msg $ intercalate "," $ sort $ fix bits gates (Map.keys gates) 0
    where (_, gates) = parseInput input
