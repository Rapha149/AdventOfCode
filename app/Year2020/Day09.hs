module Year2020.Day09 (part1, part2) where
...
part2 :: Solution
part2 input = let (len, numbers) = parseInput input
                  invalid = firstInvalid len numbers
                  n = length numbers
              in V $ hd [minimum xs + maximum xs | i <- [0..n-1], l <- [1..n-i], let xs = take l $ drop i numbers, sum xs == invalid]
