module Year2024.Day13 (part1, part2) where
...
getMinTokens resAdd limit (bA, bB, prize) | isNothing result ||
                                          ...
                                          | otherwise = a * 3 + b
    where left = (2><2) $ (\((aX, aY), (bX, bY)) -> [aX, bX, aY, bY]) $ both (tuple . map (read . drop 2) . splitOn ", " . drop 10) (bA, bB)
          right = (2><1) $ map ((+ resAdd) . read . drop 2) $ splitOn ", " $ drop 7 prize
          result = linearSolve left right
          (aD, bD) = tuple $ concat $ toLists $ fromJust result
